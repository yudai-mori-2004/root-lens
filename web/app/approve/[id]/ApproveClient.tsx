...
export default function ApproveClient({ approvalId }: { approvalId: string }) {
  ...
  return <main style={{ maxWidth: "46rem", margin: "0 auto", padding: "clamp(2rem, 7vw, 5rem) 1.25rem", color: "#111" }}>
    ...
    {details && <><p style={{ lineHeight: 1.8 }}>{details.statement}</p><dl style={{ margin: "2rem 0", padding: "1.25rem 0", borderTop: "1px solid #111", borderBottom: "1px solid #111" }}><dt style={{ fontWeight: 700 }}>撮影単位</dt><dd style={{ margin: ".4rem 0 1.25rem", overflowWrap: "anywhere" }}>{details.unitId}</dd><dt style={{ fontWeight: 700 }}>確認対象</dt><dd style={{ margin: ".4rem 0 1.25rem" }}>{details.files.map((file) => file.path).join(" / ")}</dd><dt style={{ fontWeight: 700 }}>適用される事前同意</dt><dd style={{ margin: ".4rem 0 0" }}>現場合意 {details.consentSnapshot.siteAgreementCount}件・スタッフ同意 {details.consentSnapshot.staffConsentCount}件</dd></dl><button type="button" disabled={working} onClick={approve} style={{ border: "1px solid #111", background: "#f5db74", color: "#111", padding: ".8rem 1.1rem", font: "inherit" }}>{working ? "記録中…" : "確認して提供を承認"}</button></>}
