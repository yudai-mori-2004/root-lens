...
export default function NewSiteClient({ agreement }: { agreement: string }) {
  ...
  const [siteAgreed, setSiteAgreed] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  return <div className={styles.shell}><SiteHeader /><main className={styles.main}>
    ...
    <form onSubmit={async (event) => { event.preventDefault(); setBusy(true); setError(""); try { const requestId = registrationRequestId(siteName.trim(), signerName.trim()); const response = await fetch("/api/operator/sites", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId, siteName, signerName, agreed: siteAgreed }) }); const value = await response.json(); if (response.status === 401) return router.push("/login?next=/manage/new"); if (!response.ok) return setError(value.error); sessionStorage.removeItem(registrationKey); router.push(`/manage/${value.siteId}`); } catch { setError("通信に失敗しました。同じ内容で再度お試しください。"); } finally { setBusy(false); } }}>
      ...
      <div className={styles.field}><label htmlFor="signerName">氏名</label><input id="signerName" value={signerName} onChange={(event) => { setSignerName(event.target.value); setSiteAgreed(false); }} /></div>
      ...
      {error && <p className={styles.error}>{error}</p>}<button className={styles.button} disabled={busy || !siteAgreed || !siteName.trim() || !signerName.trim()}>{busy ? "登録しています…" : "同意して事業所を登録"}</button>
    </form>
  </main></div>;
}
