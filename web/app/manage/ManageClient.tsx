...
export default function ManageClient({ sites, phoneLast4 }: { sites: Site[]; phoneLast4: string | null }) {
  ...
  const accountNames = [...new Set(sites.map((site) => site.personName))];
    ...
    <main className={styles.main}>
      ...
      <section className={styles.account} aria-label="ログイン中のアカウント">
        <div className={styles.accountHeader}>
          <div><span className={styles.accountLabel}>ログイン中のアカウント</span><strong className={styles.accountName}>{accountNames.length ? accountNames.join("・") : "SMSアカウント"}</strong></div>
        ...
        <dl className={styles.accountPhone}><dt>電話番号</dt><dd>{phoneLast4 ? `********${phoneLast4}` : "確認できません"}</dd></dl>
        {sites.length > 0 && <div className={styles.accountTableWrap}><table className={styles.accountTable}>
          <thead><tr><th>事業所</th><th>氏名</th><th>担当・役職</th><th>権限</th></tr></thead>
          <tbody>{sites.map((site) => <tr key={site.id}><td data-label="事業所">{site.name}</td><td data-label="氏名">{site.personName}</td><td data-label="担当・役職">{site.jobTitle ?? "—"}</td><td data-label="権限">{site.role === "supervisor" ? "現場監督者" : "管理者"}</td></tr>)}</tbody>
        </table></div>}
      </section>
      <section className={styles.section}>
        ...
        {sites.length === 0 ? <p className={styles.empty}>管理している事業所はありません。</p> : <div className={styles.stack}>{sites.map((site) => <div className={`${styles.card} ${styles.row}`} key={site.id}>
          <div><Link className={styles.siteNameLink} href={`/manage/${site.id}`} prefetch><strong>{site.name}</strong></Link><div className={styles.meta}>{site.role === "supervisor" ? "現場監督者" : "管理者"}</div></div>
          {site.role === "supervisor" ? <button type="button" className={styles.editLink} aria-label={`${site.name}の設定`} onClick={() => { setSettingsSiteId(site.id); setPendingSetting(null); }}><Settings size={18} strokeWidth={1.8} aria-hidden="true" />設定</button>
            : <Link className={styles.editLink} href={`/manage/${site.id}`} prefetch>開く</Link>}
        </div>)}</div>}
      </section>
      {selectedSite && <div className={styles.profileOverlay} role="dialog" aria-modal="true" aria-labelledby="siteSettingsTitle" onKeyDown={(event) => { if (event.key === "Escape") setSettingsSiteId(null); }}>
        <div className={styles.profileDialog}>
          ...
          <div className={styles.settingsList}>
            {selectedSite.siteAgreementId && <Link className={styles.settingsItem} href={`/verify/${selectedSite.siteAgreementId}`} target="_blank">現場合意の記録を確認 <span aria-hidden="true">↗</span></Link>}
