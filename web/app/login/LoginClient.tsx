...
export default function LoginClient() {
  ...
  return <div className={styles.shell}>
    ...
    <main className={`${styles.main} ${styles.narrow}`}>
      ...
      {!sent ? <form onSubmit={async (event) => { event.preventDefault(); if (await submit("/api/auth/sms/start", { phone })) setSent(true); }}>
        <div className={styles.field}><label htmlFor="phone">電話番号</label><input id="phone" inputMode="tel" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+819012345678" /></div>
        {error && <p className={styles.error}>{error}</p>}
        <button className={styles.button} disabled={busy}>{busy ? "送信中…" : "確認コードを送る"}</button>
      </form> : <form onSubmit={async (event) => { event.preventDefault(); if (await submit("/api/auth/sms/verify", { phone, code })) location.assign(next); }}>
