...
describe("desktop SMS freshness", () => {
  it("records the latest SMS verification in the signed browser cookie", async () => {
    ...
    const { issueOperatorCookie, operatorAuthenticatedAt, operatorPhoneHint } = await import("./operator-browser");
    const { issueSignedToken } = await import("./signed-token");
    const before = Date.now();
    const cookie = issueOperatorCookie("identity_test", "4015");
    ...
    expect(operatorPhoneHint(request)).toBe("4015");
    ...
    expect(operatorPhoneHint(oldRequest)).toBeNull();
  });
});
