// 開発者用の撮影・送信確認画面。
//
// 通常の撮影画面から独立した状態で、単体クリップの録画、source manifest 作成、
// アップロード、登録を個別に検証する。
...
export const DevSandboxScreen: React.FC = () => {
  ...
  // selectedCaptureMethodId = ユーザーが選んだ構成 (切替の目標)。
  // activeCaptureMethodId   = 実際に session が稼働中の構成 (= 切替完了後に更新)。preview / 録画はこちらを使う。
  ...
  // 現在 session が稼働中の captureMethod (= 次の切替で stop する対象)。null なら未稼働。
  const runningCaptureMethodRef = useRef<CaptureMethod | null>(null);

  const events = useStore(clipStore, (s) => s.events);
  const recording = useStore(clipStore, (s) => s.recording);
  // 進行中クリップ (= currentClipId が指すもの)。 段の進行 / server 状態はここにマージされる。
  const clip = useStore(clipStore, selectCurrentClip);
  const busy = useStore(clipStore, (s) => s.busy);
  ...
  // 撮影構成の session ハンドオフ。 captureMethod が変わるたびに「旧 session を完全停止 (await) →
  ...
  const runAction = useCallback(async (label: string, fn: () => Promise<void>) => {
    ...
    try {
      ...
      clipStore.getState().setBusy(null);
    }
  }, []);

  // ─── ハンドラ ─────────────────────────────────────────────────────

  // 撮影構成の切替 (= 録画待機中のみ)。 captureMethod が変わると session useEffect が
  // 旧 session を stop → 新 captureMethod で start し直す。
  const onSelectConfig = useCallback((id: string) => {
    const st = clipStore.getState();
  ...
  const onStopRecording = useCallback(() => {
    runAction('録画停止', async () => {
      ...
      const store = clipStore.getState();
      store.setSession(session);
      await enqueueRecording({ captureMethod: captureMethod, session });
  ...
  const onRunPipeline1 = useCallback(() => {
    runAction('送信 (段レジューム)', async () => {
      const clipId = clipStore.getState().currentClipId;
      if (!clipId) {
  ...
  // activeCaptureMethodId 基準にして session ハンドオフ完了まで preview を貼り替えない。
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.sub}>
          {captureMethod.label} · {recording}
          {available === false ? ' · 端末非対応' : ''}
        </Text>
        {/* 撮影構成スイッチャ (= 録画待機中のみ切替可) */}
        <View style={styles.switcher}>
          {CAPTURE_METHODS.map((c) => {
            const selected = c.id === captureMethod.id;
            const avail = captureMethodAvailability[c.id];
        ...
        {activeCaptureMethodId && PreviewView ? (
          ...
          <View style={styles.previewPlaceholder}>
            <Text style={styles.placeholderText}>
              {switching
                ? `構成切替中… (→ ${captureMethod.id})`
                : available === null
                  ? '撮影構成を確認中…'
                  : available === false
                    ? `${captureMethod.id} は当端末で利用不可`
                    : 'プレビュー起動待ち…'}
            </Text>
          </View>
        )}
      ...
      <View style={[styles.buttons, { paddingBottom: insets.bottom + 8 }]}>
        ...
        <View style={styles.row}>
          <SandboxButton label="ログクリア" onPress={() => clipStore.getState().clearEvents()} disabled={!!busy} subtle />
          <SandboxButton label="リセット" onPress={() => clipStore.getState().reset()} disabled={!!busy} subtle />
        </View>
        {busy && (
...
const LogLine: React.FC<{ event: ClipEvent }> = ({ event }) => (
