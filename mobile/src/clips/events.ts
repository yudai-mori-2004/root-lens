// Structured events emitted while a clip is prepared and submitted.
//
...
export type ClipEventInput = Omit<ClipEvent, 'id' | 'ts'>;

/** Callback a step uses to report progress. */
export type EventSink = (e: ClipEventInput) => void;

let seq = 0;

/** Stamp an input event with id / ts to make a complete ClipEvent. */
export function makeEvent(input: ClipEventInput): ClipEvent {
  seq += 1;
  return {
    ...input,
 ...
 */
export function teeToConsole(sink: EventSink, prefix = 'clips'): EventSink {
  return (e) => {
