// クリップ永続化アダプタ。
//
// clipStoreを購読し、進行中・準備完了のクリップをAsyncStorageへ保存する。
//
// 仕様:
...
import { clipStore, clipList, type Clip } from './index';
...
async function hydrate(): Promise<void> {
  ...
  clipStore.getState().replaceClips(sanitized);
}
...
export async function initClipPersistence(): Promise<void> {
  ...
  clipStore.subscribe((state, prev) => {
...
export async function retryClipPersistence(): Promise<void> {
  await persistClips(clipStore.getState().clips);
}
