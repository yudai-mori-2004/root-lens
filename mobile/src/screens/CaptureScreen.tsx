...
const CaptureBody: React.FC<Props> = ({ navigation }) => {
  ...
  // selectedCaptureMethodId = ユーザーが選んだ構成 (= 切替目標)。 activeCaptureMethodId = 実際に session が
  // 稼働中の構成 (= preview / hand-track はこちらを使う)。
  const [selectedCaptureMethodId, setSelectedCaptureMethodId] = useState<string>(DEFAULT_CAPTURE_METHOD.id);
  const [settingsLoaded, setSettingsLoaded] = useState(false);
  const [displayOrientation, setDisplayOrientation] = useState<DisplayOrientation>(
    'landscapeRight',
  );
  const [activeCaptureMethodId, setActiveCaptureMethodId] = useState<string | null>(null);
  const [switching, setSwitching] = useState(false);
  const [captureMethodAvailability, setCaptureMethodAvailability] = useState<Record<string, boolean>>({});
  // 退場中フラグ: 戻る瞬間にプレビューを隠して、 portrait へ回転する間に landscape 映像が残らないようにする。
  const [leaving, setLeaving] = useState(false);
  const captureMethod = getCaptureMethod(selectedCaptureMethodId) ?? DEFAULT_CAPTURE_METHOD;
  // session 操作 (start/stop) はカメラ排他のため直列化する (= 重ねるとクラッシュ)。
  const sessionOpRef = useRef<Promise<void>>(Promise.resolve());
  const runningCaptureMethodRef = useRef<CaptureMethod | null>(null);
  ...
  useEffect(() => {
    loadCaptureSettings()
      .then((s) => {
        const selected = getCaptureMethod(s.captureMethodId);
        if (selected) setSelectedCaptureMethodId(selected.id);
  ...
  // route. The iPhone RGB+IMU captureMethod records AAC even in gesture mode; the
  // voice flow also needs input even when ARKit is selected.
  useEffect(() => {
    if (!settingsLoaded) return;
    const allowsRecordingIOS = selectedCaptureMethodId === 'iphone'
  ...
  }, [flowId, selectedCaptureMethodId, settingsLoaded]);
  useEffect(() => () => {
  ...
  useEffect(() => {
    ...
    if (!flow.usesHardwareCaptureEvents || activeCaptureMethodId !== captureMethod.id) return;
  ...
  }, [activeCaptureMethodId, captureMethod.id, flowId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (selectedCaptureMethodId === 'iphone') {
        ...
        const entries = await Promise.all(
          CAPTURE_METHODS.map(async (c) => [c.id, await c.isAvailable().catch(() => false)] as const),
        );
        if (!cancelled) setCaptureMethodAvailability(Object.fromEntries(entries));
      } catch {
        if (!cancelled) setPermission('denied');
      }
    })();
    return () => { cancelled = true; };
  }, [settingsLoaded, selectedCaptureMethodId]);

  // 撮影構成の session ハンドオフ (= DevSandbox と同じ直列化)。 captureMethod が変わるたびに
  // 「旧 session 完全停止 (await) → カメラ解放待ち → 新 session 開始」 を直列実行する。
  // permission 許可後のみ動く。
  useEffect(() => {
    if (permission !== 'granted' || !settingsLoaded) return;
    let cancelled = false;
    const target = captureMethod;
    sessionOpRef.current = sessionOpRef.current
      .then(async () => {
        if (cancelled) return;
        const running = runningCaptureMethodRef.current;
        ...
        if (running) {
          ...
          runningCaptureMethodRef.current = null;
          setActiveCaptureMethodId(null);
        ...
        runningCaptureMethodRef.current = target;
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const subc = captureMethod.subscribeHandTrack((e) => {
  ...
  // announcing だけは「session 起動 (activeCaptureMethodId) を待つ」 + 「enter_capture の後に続ける (keep)」。
  useEffect(() => {
    ...
    // ⚠ これが無いと、 切替時に onSelectConfig が state を announcing にした瞬間、 activeCaptureMethodId が
    //    まだ旧構成 (truthy) のため handoff の enter_capture より先に TTS が積まれてしまう
    //    (= 初回オープンは activeCaptureMethodId=null から始まるので問題が出ず、 切替時だけ順序が崩れていた)。
    //    一致を要求すると、 handoff が enter_capture を積んでから setActiveCaptureMethodId(新) した後に初めて
    //    TTS が積まれ、 初回オープンと完全に同じ「enter 音 → 案内 TTS」 の順になる。
    if (state.kind === 'announcing' && (permission !== 'granted' || activeCaptureMethodId !== captureMethod.id)) return;
    if (!cue.keep) clearAudioQueue();
    if (cue.sfx) void enqueueSfx(cue.sfx);
    if (cue.tts) {
      awaitedSpeechSeqRef.current = enqueueSpeak(cue.tts);
    }
  }, [state.kind, permission, activeCaptureMethodId, captureMethod.id]);
  ...
  useEffect(() => {
    if (state.kind === 'recording' && !recordingStartedRef.current) {
      ...
      (async () => {
        try {
          const session = await captureMethod.startRecording(sink);
    ...
    if (state.kind === 'finalizing') {
      (async () => {
        try {
          ...
          const session = await captureMethod.stopRecording(sink);
          sessionDirRef.current = session.sessionDir;
    ...
    const id = setTimeout(() => {
      ...
      sessionOpRef.current = sessionOpRef.current
        .then(async () => {
          ...
          await captureMethod.startSession(sink);
          runningCaptureMethodRef.current = captureMethod;
          if (cancelled) return;
          const entrySfx = flowRef.current.sessionEntrySfx;
          if (entrySfx) enqueueSfx(entrySfx);
          setActiveCaptureMethodId(captureMethod.id);
        })
        .then(() => {
          if (!cancelled) {
            awaitedSpeechSeqRef.current = 0;
  ...
  const stopAndRegisterImmediately = useCallback(() => {
    ...
    if (stateRef.current.kind === 'finalizing') return;
    recordingStartedRef.current = false;
    const cfg = runningCaptureMethodRef.current;
    const startedAt = recordingStartedAtRef.current;
    recordingStartedAtRef.current = 0;
    if (!cfg) return;
  ...
  const onSelectConfig = useCallback((id: string) => {
    if (id === selectedCaptureMethodId) return;
    const cur = stateRef.current;
    const k = cur.kind;
    const activePhase =
  ...
  const availKnown = Object.keys(captureMethodAvailability).length > 0;
  const noConfigAvailable = availKnown && CAPTURE_METHODS.every((c) => !captureMethodAvailability[c.id]);
  ...
  // 構成が増えたらここに captureMethod id → native view の対応を足す。
  const PreviewView = activeCaptureMethodId === 'arkit'
    ? ArkitCapturePreviewView
    : activeCaptureMethodId === 'iphone'
  ...
  return (
    <View style={styles.root}>
      <View style={styles.preview}>
        ...
        ) : activeCaptureMethodId && PreviewView ? (
          <PreviewView style={StyleSheet.absoluteFill} />
        ) : (
          <View style={[StyleSheet.absoluteFill, styles.previewPlaceholder]}>
            <ActivityIndicator color={colors.accent} />
            <Text style={styles.body}>
              {switching ? t('capture.switchingConfig', { config: captureMethod.id }) : t('capture.previewStarting')}
            </Text>
          </View>
        )}
