...
2. スマートグラスをUSBで接続し、端末に残る録画をPCへ取り込んで確認する。Driveに接続できない場合も取り込みと確認は行える。
...
録画とPCに残る確認用コピーをRootLens API経由でDriveと照合します。Driveを確認できないときもPCへの取り込みと確認は進められますが、送信と削除は保留します。
