...
認証情報やフォルダIDを保持しません。RootLensセッションはアプリの実行中だけ保持し、次の起動時には再びSMS認証を行います。
...
プレビュー用録画と送信再開情報は、実行中だけPCの一時作業領域に置きます。正常終了時に削除し、異常終了後は次の起動時に古い作業領域を削除します。ログイン情報と選択中の事業所はディスクに保存しません。PCを交換しても、未送信の録画は端末から取り直せます。
