...
```bash
node tools/session_cutter/server.mjs --site-id <匿名site_id>
```
...
```bash
node tools/session_cutter/server.mjs \
  --site-id <匿名site_id> \
  --source /path/to/rec-1234567890
```

区間は追加・削除・復元のたびに、ブラウザ内の一時状態とは別に
`tools/session_cutter/boundaries/<収録名>.boundaries.json`へatomic保存します。
...
```bash
npm --prefix tools/session_cutter test
```
...
```bash
python3 tools/session_cutter/scripts/audit_historical_clock.py \
...
```bash
python3 tools/session_cutter/scripts/canonicalize_camera_imu_clock.py \
  /path/to/rec-1234567890 \
...
```bash
node tools/session_cutter/scripts/export_with_clock_audit.mjs \
  --site-id <匿名site_id> \
  --source /path/to/rec-1234567890 \
  --boundaries tools/session_cutter/boundaries/rec-1234567890.boundaries.json \
