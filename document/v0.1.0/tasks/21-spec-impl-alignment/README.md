...
- [x] コンソールログが §7.4.4 のフォーマットに準拠（%c CSSスタイリング + groupCollapsed）
...
- [x] コンソールログを %c CSSスタイリング + console.groupCollapsed に変更（ASCIIアートから移行）
- [x] アプリ側の3層証明書対応（Android/iOS ネイティブモジュール + JSブリッジ）
- [x] Dev Root CA 再生成（pathLenConstraint:1）+ iOS/Android Intermediate CA 生成
- [x] Supabase マイグレーション実行（username カラム追加）
- [x] certs/dev/README.md 追加（dev鍵がリポジトリに存在する理由を明示）
- [x] Owner ラベルの i18n 対応（en: "Owner" / ja: "所有者"）
